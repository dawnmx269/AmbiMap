...
function RealResults() {
  ...
  const { getSelections, moveToNextCategory, selectedCategories, currentCategoryIndex } = useSelection();
  const selections = getSelections();
  const realSelections = selections.filter(item => item.category === 'real');
  const [selectedChars, setSelectedChars] = useState([]);
  const [availableChars, setAvailableChars] = useState([]);
  const [resultMessage, setResultMessage] = useState(''); 
  useEffect(() => {
    // 선택된 감정어들의 message 값들을 한 글자씩 분리하여 배열로 만듦
    const chars = realSelections
      .map(item => item.selection.message.split(''))
      .flat();
    setAvailableChars(chars);

    const feelings = realSelections.map(item => item.selection.feeling);
    if (feelings.includes('허탈감') && feelings.includes('나약함')) {
      setResultMessage('혼란스러움과 소외감이 공존하는 당신은, 자신의 정체성을 찾아가는 과정에서 주변과의 거리감을 느끼고 있습니다. 이는 성장의 과정일 수 있으며, 당신만의 독특한 정체성을 만들어가는 시간입니다.');
    } else if (feelings.includes('허탈감') && feelings.includes('모순')) {
      setResultMessage('혼란스러움과 불완전함을 느끼는 당신은, 자신의 모습에 대해 깊이 고민하고 있습니다. 이러한 감정들은 당신이 더 나은 모습을 향해 나아가고 있다는 증거입니다.');
    } else if (feelings.includes('나약함') && feelings.includes('모순')) {
  ...
  const handleNext = () => {
    ...
    // 현재 카테고리의 최종 결과를 저장
    const categoryFinalResult = selectedChars.join('') + '차';
    setCategoryResult('real', categoryFinalResult);
  ...
  return (
    <div style={{ 
      ...
      <h1 style={{ marginBottom: '30px' }}>현실과의 괴리에서 느끼는 나만의 감정</h1>
      ...
      {selectedChars.length === 2 && (
        <div style={{
          ...
          <span style={{ color: '#2E7D32' }}>차</span>
        </div>
      )}
