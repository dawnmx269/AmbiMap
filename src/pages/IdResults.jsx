...
function IdResults() {
  ...
  const identitySelections = selections.filter(item => item.category === 'identity');
  ...
  useEffect(() => {
    ...
    const chars = identitySelections
      .map(item => item.selection.message.split(''))
      .flat();
    setAvailableChars(chars);

    // 선택된 감정어들의 feeling 값들을 가져와서 결과 메시지 결정
    const feelings = identitySelections.map(item => item.selection.feeling);
    if (feelings.includes('혼란스러움') && feelings.includes('소외감')) {
      setResultMessage('혼란스러움과 소외감이 공존하는 당신은, 자신의 정체성을 찾아가는 과정에서 주변과의 거리감을 느끼고 있습니다. 이는 성장의 과정일 수 있으며, 당신만의 독특한 정체성을 만들어가는 시간입니다.');
    } else if (feelings.includes('혼란스러움') && feelings.includes('불완전함')) {
  ...
  const handleNext = () => {
    ...
    // 현재 카테고리의 최종 결과를 저장
    const categoryFinalResult = selectedChars.join('') + '결';
    setCategoryResult('identity', categoryFinalResult);
  ...
  return (
    <div style={{ 
      ...
      <h1 style={{ marginBottom: '30px' }}>정체성에서 느끼는 나만의 감정</h1>
      ...
      {selectedChars.length === 2 && (
        <div style={{
          ...
          <span style={{ color: '#2E7D32' }}>결</span>
        </div>
      )}
